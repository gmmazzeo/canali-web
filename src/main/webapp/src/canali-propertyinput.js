...
(function ($) {
    ...
    var methods = {
        init: function (autocompleterMethod, queryMethod, resultDiv, options) {
            ...
            return this.each(function () {
                ...
                $(this).data("propertyInputObject", new $.PropertyList(this, autocompleterMethod, queryMethod, resultDiv, settings));
            });
        },
        clear: function () {
            this.data("propertyInputObject").clear();
            return this;
        },
        add: function (item) {
            this.data("propertyInputObject").add(item);
            return this;
        },
        remove: function (item) {
            this.data("propertyInputObject").remove(item);
            return this;
        },
        get: function () {
            return this.data("propertyInputObject").getTokens();
        },
        ...
        destroy: function () {
            if (this.data("propertyInputObject")) {
                this.data("propertyInputObject").clear();
                var tmpInput = this;
    ...
    $.fn.propertyInput = function (autocompleteMethod, queryMethod, resultDiv) {
        return methods.init.apply(this, arguments);
    };

    // PropertyList class for each input
    $.PropertyList = function (input, autocompleteMethod, queryMethod, resultDiv) {
        ...
        function showResults(result) {
            ...
            if (result[0][0][0] === "Error") {
                div.text(result[0][1][0]);
            } else {
                div.html("");
                div.append($('<div><b>Domain</b><br /></div>'));                
                var c="";
                $.each(result[0][0], function (index, value) {
                    c=c+value+"; "
                });
                if (c) {
                    div.append($('<div><u>Classes</u>: '+c+'<br /></div>'));
                }
                var a="";
                $.each(result[0][1], function (index, value) {
                    a=a+value+"; "
                });
                if (a) {
                    div.append($('<div><u>Properties</u>: '+a+'<br /></div>'));
                }
                div.append($('<div><b>Range</b><br /></div>'));                
                c="";
                $.each(result[1][0], function (index, value) {
                    c=c+value+"; "
                });
                if (c) {
                    div.append($('<div><u>Classes</u>: '+c+'<br /></div>'));
                ...
                a="";
                $.each(result[1][1], function (index, value) {
                    a=a+value+"; "
                });
                if (a) {
                    div.append($('<div><u>Properties<u>: '+a+'<br /></div>'));
                ...
                var t="";
                $.each(result[1][2], function (index, value) {
                    t=t+value+"; "
                });
                if (a) {
                    div.append($('<div><u>Data types</u>: '+t+'<br /></div>'));
                }
            ...
            run_search_generic(query, function (results, query) {
                ...
                if (!results || results.length === 0) {
                    if (query.length === 0 && saved_tokens.length === 0) {
                        show_dropdown_message("Start typing an property name");
                    } else if (query.length < 4) {
                        show_dropdown_message("Continue typing the property name");
                    } else {
        ...
        function do_query() {
            ...
            ajax_params.data = {};
            var states = [];
            var tokens = $(input).data("propertyInputObject").getTokens();
            var t = {};
